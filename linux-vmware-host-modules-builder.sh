...
function showScriptInfo(){
    ...
    cPrint "GREEN" "Vmware host modules downloaded from https://github.com/mkubecek/vmware-host-modules\n."
}
...
function getTargetVmwareVersion(){
  while true; do # Start infinite loop
      ...
      # Check input
      if [[ $targetVmwareVersion =~ $versionNoSequence ]] ; then
         return $(true) # Exit loop returning true
      else cPrint "GREEN" "Invalid version!! Please try again." # Invalid entry
      fi
      sleep 1 # Hold loop
  done
}
...
function downloadHostModules(){
    if getTargetVmwareVersion; then
...
function initScript(){
    ...
    if isUserRoot; then
        if isConnected; then # Network connection established
